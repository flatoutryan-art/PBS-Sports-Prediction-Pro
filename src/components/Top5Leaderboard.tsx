...
interface LeaderRow { id: string; display_name: string | null; username: string; total_points: number }

async function fetchTop5(): Promise<LeaderRow[]> {
...
export default function Top5Leaderboard({ currentUserId, variant }: Props) {
  const { data } = useQuery({ queryKey: ['top5'], queryFn: fetchTop5, staleTime: 60_000 })
    ...
    <div className={clsx(variant === 'strip' ? 'flex gap-2 overflow-x-auto scrollbar-hide' : 'space-y-2')}>
      {data.map((row, i) => (
        <div key={row.id} className={clsx(
          ...
          row.id === currentUserId && 'border-gold/30'
        )}>
